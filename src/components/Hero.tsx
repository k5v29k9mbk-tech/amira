    ...
    <section className="hero relative isolate overflow-hidden bg-ivory">
      <div className="mx-auto grid w-full max-w-[1920px] lg:min-h-[100svh] lg:grid-cols-[1.1fr_0.9fr] lg:grid-rows-[1fr_auto]">
        ...
        {/* THE PORTRAIT.
            ...
            the composition from lg; a 4:5 plate under the copy on a phone,
            which is the proportion that holds her head and shoulders without
            cropping either. `heroMedia` carries the crop for both, aimed high
            because a portrait's subject is its face and the space to lose is
            under it.
        ...
        <div className="relative aspect-[4/5] w-full sm:aspect-[3/2] lg:col-start-2 lg:row-span-2 lg:row-start-1 lg:aspect-auto lg:h-full">
          <MediaFrame
          ...
          {/* The credit, set the way a magazine credits the subject of a
              photograph rather than the way a website captions an image: her
              name at reading size, the three titles under it in small caps.

              The scrim is a short, weak gradient at the foot of the frame and
              nothing else. The bottom of this photograph is a black blazer, so
              ivory type on it already clears contrast comfortably; the gradient
              is there for the crop on a phone, where the frame cuts higher and
              the type can land on skin. It stops at 38% so it never reads as a
              band across the picture. */}
          <div
            aria-hidden
            className="pointer-events-none absolute inset-x-0 bottom-0 h-[38%] bg-gradient-to-t from-espresso/70 via-espresso/25 to-transparent"
          />
          <HeroBeat
            delay={heroBeat.name}
            className="absolute inset-x-0 bottom-0 px-6 pb-8 md:px-10 md:pb-10 lg:pb-12 lg:ps-12 xl:ps-16"
          >
            <p className="text-[17px] leading-tight text-ivory md:text-[19px]">
              {inst("title")}
            </p>
            <p className="label mt-2.5 leading-[1.6] text-ivory/75">
              {t("founderRole")}
            </p>
          </HeroBeat>
        </div>
        ...
        {/* THE PROOF.
            A masthead band rather than a table: one hairline over three figures
            on a shared baseline, no rules between them and no box around any of
            them. Three columns from 320px up, because set inline the three
        ...
        >
          <dl className="grid grid-cols-3 items-start gap-x-4 border-t border-hair pt-[var(--hero-air-md)] sm:gap-x-8">
            {facts.map((k) => {
              /* The class size is the one figure here that is a promise about
                 the room rather than a record of the past, and it is the reason
                 the training is what it is. It is the only one in bronze and
                 the only one carrying a line of its own. */
              const lead = k === "classes";
              return (
                <div key={k} className="flex flex-col gap-[var(--hero-air-xs)]">
                  <dt
                    className={`display text-[1.75rem] leading-none md:text-[2.25rem] ${
                      lead ? "text-bronze-ink" : "text-espresso"
                    }`}
                  >
                    {t(`facts.${k}.value`)}
                  </dt>
                  <dd className="label fact-label text-balance leading-[1.45] text-mute">
                    {t(`facts.${k}.label`)}
                  </dd>
                </div>
              );
          ...
          {/* The sentence the third figure implies, said once. It sits under the
              band rather than inside the column so the three figures keep one
              baseline and the row stays a masthead. */}
          <p className="mt-[var(--hero-air-sm)] max-w-[42ch] text-[14px] leading-relaxed text-mute md:text-[15px]">
            {t("classesNote")}
          </p>
        </HeroBeat>
      </div>
