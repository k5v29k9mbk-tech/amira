...
const HOMEPAGE_KEYS = [
  ...
  ...["academy", "certification", "groups", "support"].flatMap((k) => [
    `proof.${k}.title`,
    `proof.${k}.sub`,
  ]),
  ...["eyebrow", "quote", "play", "videoSoon", "videoAlt"].map((k) => `mentor.${k}`),
  ...["eyebrow", "title", "sub"].map((k) => `journey.${k}`),
  ...["book", "welcome", "theory", "demo", "practice", "certify"].flatMap((k) => [
    `journey.steps.${k}.title`,
    `journey.steps.${k}.body`,
  ]),
  ...["eyebrow", "title", "sub"].map((k) => `brand.${k}`),
  ...["identity", "pricing", "social", "photography", "clients", "systems"].flatMap((k) => [
    `brand.items.${k}.title`,
    `brand.items.${k}.body`,
  ]),
